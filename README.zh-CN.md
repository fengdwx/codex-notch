...
即使没有任务正在运行，周额度指标也会一直保留在实体刘海右侧；有 5 小时窗口时，5h 指标会显示在左侧。没有 5h 数据时左侧保持空白，展开卡片则先显示 5h 明细，再显示周额度。切到浏览器、IDE 或其他应用，额度仍然可见。
...
额度窗口根据接口返回的 `limit_window_seconds` 动态识别；实体刘海中周额度继续显示在紧凑态右侧，返回 5 小时窗口时左侧显示 5h，展开卡片先显示 5h 明细。没有 5h 数据时左侧保持空白；无刘海浮动条保留独立的状态位。
